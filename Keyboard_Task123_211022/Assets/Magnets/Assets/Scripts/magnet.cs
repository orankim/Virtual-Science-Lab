...
public class magnet : MonoBehaviour {
    ...
    void attract(Rigidbody rbToAttract, Rigidbody magnetRb)
    {
        ...
        if (rbToAttract.mass > magnetRb.mass)
        {
            ...
            magnetRb.AddForce(-(magnetRb.position - rbToAttract.position) * magnetPower,ForceMode.Force);
            //returns
            return;
        }

        //add force to the rigidbody that must be attracted
        rbToAttract.AddForce((magnetRb.position - rbToAttract.position) * magnetPower * rbToAttract.mass * Time.deltaTime, ForceMode.Force);
    }
